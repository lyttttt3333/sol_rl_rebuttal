...
results promising and consistent. The concerns identified in the meta-review have helped us
sharpen the scope and substantially strengthen the evaluation. We respond below in the same
order as the meta-review.

---

## 1. Over-reliance on reward models and fragile FP4 ranking assumptions

### 1.1 Robustness across reward-model types

The core requirement of Sol-RL is not that FP4 reproduce BF16 image quality, but that it
preserve enough relative reward ordering to select informative candidates. From a pool of 96
candidates, we take the FP4 proxy's top-12 and bottom-12 and report their average ranks under
the BF16 reward. A perfect proxy gives **6.5 / 90.5**; a proxy carrying no information gives
**48.5 / 48.5**.
...
| Aesthetic | low-level aesthetic | 9.3 / 86.7 | 11.7 / 84.4 | 15.1 / 79.1 |
| OCR / text fidelity | clarity-based | 13.2 / 82.8 | 12.7 / 83.2 | 13.0 / 80.9 |
| *Perfect proxy* | | *6.5 / 90.5* | *6.5 / 90.5* | *6.5 / 90.5* |
| *No-information proxy* | | *48.5 / 48.5* | *48.5 / 48.5* | *48.5 / 48.5* |

- Ranking fidelity is strongest for human-preference rewards and lower for low-level
  aesthetic and clarity-based rewards, consistent with the reviewers' hypothesis.
- Nevertheless, every tested model-reward combination remains clearly informative relative
  to the no-information reference. Even the weakest cell, SD3.5/Aesthetic at
  **15.1 / 79.1**, is far from **48.5 / 48.5**.
- We will revise the paper to delimit this assumption rather than claim uniform validity.

### 1.2 Does FP4 noise amplify incorrect preferences?

The two-stage design prevents FP4 scores from becoming training labels. FP4 is used only to
select seeds; those seeds are regenerated in BF16 and independently re-scored before
optimization. FP4 error can therefore reduce how informative the selected batch is, but it
does not create an additional pathway for incorrect FP4 preferences to enter the gradient.

To isolate FP4-induced noise, we vary only the first-stage precision:
...
The ranking perturbation is measurable, but remains far from random. More importantly, it
does not propagate into worse end-to-end performance under equal compute:
...
### 1.3 Genuine alignment versus reward hacking

Like other reward-based RL post-training methods, Sol-RL optimizes a learned reward model and
therefore inherits its limitations. However, the qualitative comparisons in **Figure 5**
show corresponding improvements in visual quality rather than obvious reward hacking. We are
also conducting an internal human-preference study and will report the results during the
discussion period.
...
We agree that the rollout bottleneck and degradation caused by low-precision training are
established observations in quantized RL for LLMs. However, quantization in
diffusion RL remains comparatively underexplored. We thank the reviewer for this correction
and will revise the established observations as motivation and state our contribution more
precisely.

### 2.2 Significance and statistical validation

Comparing only final raw rewards understates the improvement because each metric has a
substantial nonzero starting value. The relevant quantity is the gain from the shared base
model, $\Delta R=R_{\mathrm{final}}-R_{\mathrm{base}}$. Under the identical GPU-hour budget
in Table 1, DiffusionNFT versus Sol-RL obtains:

| Metric | DiffusionNFT $\Delta R$ | Sol-RL $\Delta R$ | Relative increase |
|---|---|---|---|
| ImageReward | 1.2157 | **1.3086** | **7.6%** |
| CLIPScore | 0.0361 | **0.0459** | **27.1%** |
| PickScore | 0.0756 | **0.0836** | **10.6%** |
| HPSv2 | 0.1047 | **0.1122** | **7.2%** |

We additionally ran four independent SD3.5 training runs for CLIPScore and PickScore:
...
The low variance demonstrates that training is stable and that the observed gains are
consistent across runs. We will include comprehensive variance reporting in the revision.

### 2.3 Speedup target selection

We agree that a single target taken from the baseline's late-stage plateau can inflate the
apparent speedup. We therefore divide each baseline trajectory into 25%, 50%, 75%, and 100%
progress milestones and report time-to-reward at every milestone:
...
The speedup never falls below **1.6×**, so the benefit is not confined to the plateau. We
will report the full trajectory rather than emphasize only the largest endpoint value.

### 2.4 Missing analysis details and robustness breakdowns

- **Prompts:** 256 from the standard DiffusionNFT splits.
- **Seeds:** 96 per prompt, totaling **24,576 samples per configuration**.
- **Candidates:** 96 per group; cells report the BF16 mean rank of the FP4-selected top-12
  and bottom-12.
- **Base models:** FLUX.1, SANA, and SD3.5. The original paper pooled the analysis; the
  rebuttal disaggregates it.
- **Reward models:** HPSv2, PickScore, ImageReward, Aesthetic, and OCR.

*GenEval categories, from single-object to position:*
...
*OCR difficulty, from one to five digits:*
...
*Sampling-schedule robustness on FLUX.1:*
...
Ranking fidelity remains stable across prompt difficulty and changes only modestly across
sampling schedules.

### 2.5 Experimental scope beyond DiffusionNFT

We applied the identical two-stage pipeline to Flow-GRPO and online DPO, changing only the
policy-optimization objective. Setup: SD3.5, HPSv2, 512px, LoRA, 8×H100, and a held-out
2048-prompt evaluation set.
